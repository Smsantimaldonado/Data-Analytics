...
SELECT Documento, nombre, Camada, 'Mayor' as marca
FROM staff
WHERE Camada = (SELECT MAX(Camada) FROM staff)
UNION
SELECT Documento, nombre, Camada, 'Menor' as marca
FROM staff 
...
FROM staff S
			INNER JOIN Encargado E ON E.Encargado_ID = S.Encargado
...
FROM Profesiones P
				  LEFT JOIN Estudiantes E ON P.ProfesionesID = E.Profesion
...
FROM Asignaturas A
				  ...
				  LEFT JOIN Staff S ON S.Asignatura = A.AsignaturasID
				  LEFT JOIN Estudiantes E ON S.DocentesID = E.Docente
...
FROM Estudiantes E
				  INNER JOIN Staff S ON E.Docente = S.DocentesID
...
FROM Encargado E
				INNER JOIN Staff S ON E.Encargado_ID = S.Encargado
				INNER JOIN Asignaturas A ON A.AsignaturasID = S.Asignatura
...
SELECT A.Nombre,
	   ...
	   B.AvgCosto
FROM Asignaturas A
				  LEFT JOIN (SELECT Area, avg(Costo) as AvgCosto
							 FROM  Asignaturas
							 GROUP BY Area) B   
ON A.Area = B.Area   
...
FROM Staff S
			 LEFT JOIN Asignaturas A ON S.Asignatura = A.AsignaturasID
			 INNER JOIN area B ON A.Area = B.AreaID
